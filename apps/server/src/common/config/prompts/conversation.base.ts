...
export const buildConversationBasePrompt = (
  ...
  return [
    ...
    `The reply field is the dialogue bubble the learner sees first, so keep reply in ${targetLabel} unless a later mode rule explicitly narrows it further.`,
    ...
    `reply -> keep it in ${targetLabel} and make it sound like a real in-scene human response; native-language coaching belongs in correction/scoreReason/tips instead of reply;`,
