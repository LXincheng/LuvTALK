...
export class VoiceTutorService {
  ...
  async handleUpload(
    ...
    void this.processVoiceUpload(conversationId, result, languageHint).catch(
      (error) => {
        this.logger.error(
          `Failed to process voice upload ${result.operationId}`,
  ...
  private async transcribeWithOpenAi(
    ...
    if (!apiKey || !transcribeModel || !apiUrl) {
      this.logger.warn("OpenAI 配置缺失，无法执行语音识别");
      return undefined;
    }
    try {
      ...
      if (languageHint?.languageCode) {
        formData.append("language", languageHint.languageCode);
      }
      if (languageHint?.transcriptionPrompt) {
      ...
      this.logger.error(
        `OpenAI transcription exception for ${upload.operationId}`,
        error instanceof Error ? error.stack : String(error),
      );
      return undefined;
    }
  ...
  async synthesizeSpeech(
    ...
    if (!apiKey || !ttsModel || !apiUrl) {
      this.logger.warn("OpenAI 配置缺失，无法执行语音合成");
      return undefined;
    }
    const session = await this.conversationService.getSession(conversationId);
    const languageHint = this.resolveLanguageHint(session.targetLanguage);
    const directory = join(this.storageRoot, conversationId);
    try {
      const response = await fetch(`${apiUrl.replace(/\/$/, "")}/audio/speech`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${apiKey}`,
      ...
      this.logger.error(
        `OpenAI speech synthesis exception for conversation ${conversationId}`,
        error instanceof Error ? error.stack : String(error),
      );
      return undefined;
    }
  ...
  private resolveLanguageHint(language?: LanguageCode | string): LanguageHint {
    switch (language) {
      ...
      case LanguageCode.Cantonese:
        return {
          languageCode: "zh",
          transcriptionPrompt: "说话人使用粤语（Cantonese Chinese），请按粤语发音转写。",
          ttsVoice: "fable",
        };
