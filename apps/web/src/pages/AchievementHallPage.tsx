...
import { RARITY_COLORS, RARITY_GLOW, type RarityType } from '../constants/ui';
...
export default function AchievementHallPage() {
  ...
  // Rarity translation key mapping
  ...
  const achievements: Achievement[] = [
    {
      id: '1',
      title: t('achievementFirstStepsTitle'),
      description: t('achievementFirstStepsDesc'),
      icon: '💬',
    ...
    {
      id: '2',
      title: t('achievementWeekWarriorTitle'),
      description: t('achievementWeekWarriorDesc'),
      icon: '🔥',
    ...
    {
      id: '3',
      title: t('achievementVocabMasterTitle'),
      description: t('achievementVocabMasterDesc'),
      icon: '📚',
    ...
    {
      id: '4',
      title: t('achievementPerfectPronunciationTitle'),
      description: t('achievementPerfectPronunciationDesc'),
      icon: '🎯',
      color: 'from-purple-500 to-pink-500',
      progress: 0,
      total: 1,
    ...
    {
      id: '5',
      title: t('achievementSocialButterflyTitle'),
      description: t('achievementSocialButterflyDesc'),
      icon: '🦋',
    ...
    {
      id: '6',
      title: t('achievementSpeedLearnerTitle'),
      description: t('achievementSpeedLearnerDesc'),
      icon: '⚡',
    ...
    {
      id: '7',
      title: t('achievementPolyglotTitle'),
      description: t('achievementPolyglotDesc'),
      icon: '🌍',
    ...
    {
      id: '8',
      title: t('achievementDiamondStreakTitle'),
      description: t('achievementDiamondStreakDesc'),
      icon: '💎',
  ...
  const currentXP = 720;
  const currentLevel = levels.filter((level) => level.unlocked).length;
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 md:py-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-semibold text-slate-900 dark:text-white mb-2">
            {t('achievementHallTitle')}
          </h1>
          <p className="text-slate-600 dark:text-slate-400">
            {t('achievementHallSubtitle')}
          </p>
        </div>
        ...
        <div className="flex gap-2 mb-6 glass-card rounded-xl p-1 border border-slate-200 dark:border-slate-700">
          <button
            ...
            className={`flex-1 px-4 md:px-6 py-2.5 md:py-3 rounded-lg font-medium transition-all text-sm md:text-base ${
...
function AchievementCard({
  ...
  t: (key: string) => string;
  getRarityTranslation: (rarity: 'common' | 'rare' | 'epic' | 'legendary') => string;
  delay: number
}) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay }}
      className={`glass-card rounded-xl p-4 md:p-5 border-2 ${
        achievement.unlocked
          ? `border-${achievement.rarity === 'common' ? 'slate' : achievement.rarity === 'rare' ? 'blue' : achievement.rarity === 'epic' ? 'purple' : 'yellow'}-400 dark:border-${achievement.rarity === 'common' ? 'slate' : achievement.rarity === 'rare' ? 'blue' : achievement.rarity === 'epic' ? 'purple' : 'yellow'}-600 shadow-lg ${RARITY_GLOW[achievement.rarity]}`
    ...
    >
      ...
      <div className="relative">
        <div className={`text-3xl md:text-4xl mb-3 ${achievement.unlocked ? '' : 'grayscale opacity-50'}`}>
          {achievement.icon}
        </div>

        <h3
          className={`font-semibold mb-1 text-sm md:text-base ${
            achievement.unlocked ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-600'
          }`}
        >
          {achievement.title}
        </h3>
...
function LevelCard({
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      className={`glass-card rounded-xl p-4 md:p-6 border-2 ${
        level.unlocked
    ...
    >
      <div className="flex items-center gap-4 md:gap-6">
        <div className={`text-4xl md:text-5xl flex-shrink-0 ${level.unlocked ? '' : 'grayscale opacity-50'}`}>
          {level.icon}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 md:gap-3 mb-2">
            <span className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white">
              {t('achievementLevel')}{level.level}
            </span>
            <span
              className={`text-base md:text-lg font-semibold truncate ${
                level.unlocked ? 'text-slate-700 dark:text-slate-300' : 'text-slate-500 dark:text-slate-600'
              }`}
            >
              {level.title}
            </span>
            {level.unlocked ? (
              <div className="ml-auto flex-shrink-0">
                <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-green-100 dark:bg-green-950 flex items-center justify-center">
                  <span className="text-sm md:text-base">✓</span>
                </div>
              </div>
            ) : (
              <div className="ml-auto flex-shrink-0">
                <Lock className="w-5 h-5 md:w-6 md:h-6 text-slate-400 dark:text-slate-600" />
              </div>
            )}
          </div>
          ...
          <p className="text-xs md:text-sm text-slate-600 dark:text-slate-400 mb-2 md:mb-3">
            {level.unlocked
              ? t('achievementUnlockedAt').replace('{xp}', level.minXP.toString())
              : t('achievementRequires').replace('{xp}', level.minXP.toString())}
          </p>
            ...
            <div className="space-y-1.5 md:space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-slate-500 dark:text-slate-500">
                  {t('achievementToLevel').replace('{level}', (level.level + 1).toString())}
                </span>
                <span className="font-medium text-slate-700 dark:text-slate-300">
                  {xpProgress}/{xpNeeded} {t('achievementXP')}
                </span>
              </div>
        ...
        {nextLevel && (
          <ChevronRight
            className={`w-5 h-5 md:w-6 md:h-6 flex-shrink-0 hidden md:block ${
              level.unlocked ? 'text-slate-400 dark:text-slate-600' : 'text-slate-300 dark:text-slate-700'
            }`}
          />
        )}
      </div>
