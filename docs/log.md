...
- 之前问题根因：总结卡把“动作区 + 信息区”都放在单行弹性布局，窄屏和长文案下容易发生挤压、换行错位、内容跳动。
- 修复策略：优先使用“可预测”的布局（移动端列布局、按钮网格、卡片最小宽度约束）而不是依赖单一 flex 自动分配。
- 经验沉淀：对高密度信息卡片，必须同时提供“摘要态 + 详情态”；并在组件层默认开启溢出保护（`min-w-0`、`break-words`、`truncate`）。
- 验证结果：`pnpm exec eslint`（Summary/Conversation 相关文件）通过，`pnpm --filter web exec tsc -b` 通过。
